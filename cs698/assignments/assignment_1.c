...
int main(int argc, char *argv[]) {
    ...
    /* Use MPI_Send */
    /* Leaving the first elms_to_comm or rows [0, my_work) for ROOT*/
        for(i = 1; i < num_procs; i++){
            int ro = i * my_work;
            MPI_Send(&A[ro*n], elms_to_comm, MPI_DOUBLE, i, 0, world);
        }
    } else {
    /* Receive my_work rows of A */
    /* Use MPI_Recv */
        MPI_Recv(&A[elem_offset], elms_to_comm, MPI_DOUBLE, ROOT, 0, world, &status);
    }

    /* Broadcast B to every one */
    if (my_rank == ROOT) {
        for (i = 1; i < num_procs; i++){
            MPI_Send(B, n_sq, MPI_DOUBLE, i, 1, world);
        }
    } else {
        MPI_Recv(B, n_sq, MPI_DOUBLE, ROOT, 1, world, &status);
    }
    ...
    /* Use MPI_Recv */
        for (i = 1; i < num_procs; i++){
            int ro = i * my_work;
            MPI_Recv(&C[ro*n], elms_to_comm, MPI_DOUBLE, i, 2, world, &status);
        }
    } else {
    /* Send my_work rows of C to ROOT */
    /* Use MPI_Send */
        MPI_Send(&C[elem_offset], elms_to_comm, MPI_DOUBLE, ROOT, 2, world);
    }
